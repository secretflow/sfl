...
def comprehensive_distribution_test(
    sample1: np.ndarray, sample2: np.ndarray, alpha: float = 0.01
) -> Dict:
...
def _determine_overall_conclusion(results: Dict, alpha: float):
    ...
    different_count = sum(
        1 for test in tests.values() if test.get("distributions_different", False)
    )
    ...
    if total_tests > 0:
        if different_count == 0 and stats_ok:
            ...
            results["overall_conclusion"] = (
                f"Distributions appear similar (0/{total_tests} tests significant, stats within tolerance)"
            )
        elif different_count > 0:
            results["distributions_are_different"] = True
            results["overall_conclusion"] = (
                f"Evidence of different distributions ({different_count}/{total_tests} tests significant)"
            )
        else:
            results["distributions_are_different"] = False
            results["overall_conclusion"] = (
                f"Distributions appear similar ({different_count}/{total_tests} tests significant, stats within tolerance)"
            )
    else:
    ...
    def test_array_shapes(self):
        ...
        for shape in shapes:
            expected_shape = (
                ...
                else tuple([shape] if isinstance(shape, int) else shape)
            )
