...
@mplang.function
def linear_model_predict(model, X:Dict[int, MPObject]) -> MPObject:
    """
    ...
    y_pred_party = {party_id:simp.runAt(party_id,lambda x,w: x @ w)( x, model.weights[party_id]) for party_id, x in X.items()}
    y_pred_no_intercept = smpc.revealTo(MPAggregator.sum(y_pred_party), model.intercept_party)
    if model.intercept is not None:
        return simp.runAt(model.intercept_party,lambda x, b: x + b)(y_pred_no_intercept, model.intercept)
...
if __name__ == "__main__":
    ...
    # 在不同参与方生成权重向量(jax数组)
    party0_weight = simp.runAt(0, lambda: jnp.array([random.uniform(0.5, 1.5) for _ in range(n_features)]))()  # P0权重向量
    party1_weight = simp.runAt(1, lambda: jnp.array([random.uniform(0.5, 1.5) for _ in range(n_features)]))()  # P1权重向量
    ...
    # 在不同参与方生成输入数据(jax数组)
    n_samples = 1
    party0_X = simp.runAt(0, lambda: jnp.array([[random.uniform(0.5, 2.0) for _ in range(n_features)] for _ in range(n_samples)]))()  # P0输入矩阵
    party1_X = simp.runAt(1, lambda: jnp.array([[random.uniform(0.5, 2.0) for _ in range(n_features)] for _ in range(n_samples)]))()  # P1输入矩阵
